...
local function CDs ()
  ...
  if Target:IsInMeleeRange(5) then
    ...
    if Target:Level() == 62 and (Player:IsInRaidArea() or Player:IsInDungeonArea()) or Target:Level() >= 61 and not Player:IsInRaidArea() and not Player:IsInDungeonArea() and S.Flagellation:IsReady() and SnDCondition and not Player:StealthUp(true, true) then
  ...
  if Target:Level() == 62 and (Player:IsInRaidArea() or Player:IsInDungeonArea()) or Target:Level() >= 61 and not Player:IsInRaidArea() and not Player:IsInDungeonArea() and S.Vanish:IsCastable()
  ...
  if Target:IsInMeleeRange(5) then
    ...
    if Target:Level() == 62 and (Player:IsInRaidArea() or Player:IsInDungeonArea()) or Target:Level() >= 61 and not Player:IsInRaidArea() and not Player:IsInDungeonArea() and S.Sepsis:IsReady() and SnDCondition and ComboPointsDeficit >= 1 then
  ...
  if S.MarkedforDeath:IsCastable() then
    ...
    if not Player:StealthUp(true, true) and ComboPointsDeficit >= Rogue.CPMaxSpend() then
      ...
      elseif Target:Level() == 62 and (Player:IsInRaidArea() or Player:IsInDungeonArea()) or Target:Level() >= 61 and not Player:IsInRaidArea() and not Player:IsInDungeonArea() then
        if HR.Cast(S.MarkedforDeath, Settings.Commons.OffGCDasOffGCD.MarkedforDeath) then return "Cast Marked for Death" end
      end
    end
  end
...
local function Stealth_CDs (EnergyThreshold)
  if Target:Level() == 62 and (Player:IsInRaidArea() or Player:IsInDungeonArea()) or Target:Level() >= 61 and not Player:IsInRaidArea() and not Player:IsInDungeonArea() and S.ShadowDance:TimeSinceLastDisplay() > 0.3 and S.Shadowmeld:TimeSinceLastDisplay() > 0.3 and not Player:IsTanking(Target) then
  ...
  if ShD_Combo_Points() and Target:IsInMeleeRange(5) and S.ShadowDance:IsCastable() and S.ShadowDance:Charges() >= 1
    ...
    and (Target:Level() == 62 and (Player:IsInRaidArea() or Player:IsInDungeonArea()) or Target:Level() >= 61 and not Player:IsInRaidArea() and not Player:IsInDungeonArea() or (S.ShadowDance:ChargesFractional() >= Settings.Subtlety.ShDEcoCharge - (not S.EnvelopingShadows:IsAvailable() and 0.75 or 0))) then
...
local function APL ()
  ...
  if not Player:AffectingCombat() then
    ...
    if not Player:BuffUp(S.ShadowDanceBuff) and not Player:BuffUp(S.SoulShape) and not Player:BuffUp(VanishBuff) and not IsResting() and not Player:IsCasting() and not IsMounted() then
