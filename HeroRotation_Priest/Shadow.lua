...
local function Precombat()
  ...
  -- PW: Shield
  if S.PWShield:IsCastable() and Player:IsMoving() and not Player:IsInRaidArea() and not Player:IsInDungeonArea() and Player:DebuffDown(S.WeakenedSoul) then
    if Cast(S.PWShield) then return "Power Word: Shield"; end
  end
  -- flask
  -- food
  -- augmentation
  ...
  if Everyone.TargetIsValid() then
    ...
    -- arcane_torrent
    if S.ArcaneTorrent:IsCastable() and Target:Level() == Player:Level() + 2 and (Player:IsInRaidArea() or Player:IsInDungeonArea()) or Target:Level() == Player:Level() + 1 and not Player:IsInRaidArea() and not Player:IsInDungeonArea() then
...
local function Main()
  ...
  if S.MindSear:IsCastable() and (S.SearingNightmare:IsAvailable() and EnemiesCount10ySplash > VarMindSearCutoff and UnitsWithoutSWPain > 0 and not S.Mindbender:CooldownUp() or IsShiftKeyDown()) then
  ...
  if S.Mindbender:IsCastable() and Target:Level() == Player:Level() + 2 and (Player:IsInRaidArea() or Player:IsInDungeonArea()) or Target:Level() == Player:Level() + 1 and not Player:IsInRaidArea() and not Player:IsInDungeonArea() and (Target:DebuffUp(S.VampiricTouchDebuff) and (S.SearingNightmare:IsAvailable() and EnemiesCount10ySplash > VarMindSearCutoff or Target:DebuffUp(S.ShadowWordPainDebuff))) then
  ...
  if S.MindSear:IsCastable() and ((EnemiesCount10ySplash > VarMindSearCutoff and Player:BuffUp(S.DarkThoughtBuff)) or IsShiftKeyDown()) then
  ...
  if S.MindFlay:IsCastable() and (not Player:IsChanneling() or Player:IsChanneling() and Player:ChannelRemains() < 0.1 ) and (Player:BuffUp(S.DarkThoughtBuff) and not (Player:BuffUp(S.VoidformBuff) and S.VoidBolt:CooldownRemains() <= Player:GCD()) and VarDotsUp) then
  ...
  if S.MindSear:IsCastable() and ((EnemiesCount10ySplash > VarMindSearCutoff) or IsShiftKeyDown()) then
    if Cast(S.MindSear, nil, nil, not Target:IsSpellInRange(S.MindSear)) then return "mind_sear 130"; end
  end
  -- mind_flay,chain=1,interrupt_immediate=1,interrupt_if=ticks>=2&(cooldown.void_bolt.up|cooldown.mind_blast.up)
  if S.MindFlay:IsCastable() and (not Player:IsChanneling() or Player:IsChanneling() and Player:ChannelRemains() < 0.1 ) then
...
local function APL()
  ...
  if Everyone.TargetIsValid() and (not Player:IsChanneling(S.VoidTorrent) or Player:IsChanneling(S.VoidTorrent) and Player:ChannelRemains() < 0.1 )then
