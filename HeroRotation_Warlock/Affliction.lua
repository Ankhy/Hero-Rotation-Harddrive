...
local function Aoe()
  ...
  if S.SummonDarkglare:IsReady() and Target:Level() == Player:Level() + 2 and (Player:IsInRaidArea() or Player:IsInDungeonArea()) or Target:Level() == Player:Level() + 1 and not Player:IsInRaidArea() and not Player:IsInDungeonArea() and (Target:DebuffRemains(S.PhantomSingularityDebuff) > 2 or not S.PhantomSingularity:IsAvailable()) then
  ...
  if S.DarkSoulMisery:IsReady() and Target:Level() == Player:Level() + 2 and (Player:IsInRaidArea() or Player:IsInDungeonArea()) or Target:Level() == Player:Level() + 1 and not Player:IsInRaidArea() and not Player:IsInDungeonArea() and (S.SummonDarkglare:CooldownRemains() > Target:TimeToDie()) then
...
local function APL()
  ...
  if Everyone.TargetIsValid() then
    ...
    if S.SummonDarkglare:IsReady() and Target:Level() == Player:Level() + 2 and (Player:IsInRaidArea() or Player:IsInDungeonArea()) or Target:Level() == Player:Level() + 1 and not Player:IsInRaidArea() and not Player:IsInDungeonArea() and (Enemies40yCount > 2 and (Target:DebuffUp(S.PhantomSingularityDebuff) or not S.PhantomSingularity:IsAvailable())) then
      local ShouldReturn = Darkglare_prep(); if ShouldReturn then return ShouldReturn; end
    end
    ...
    if Target:Level() == Player:Level() + 2 and (Player:IsInRaidArea() or Player:IsInDungeonArea()) or Target:Level() == Player:Level() + 1 and not Player:IsInRaidArea() and not Player:IsInDungeonArea() and (S.SummonDarkglare:CooldownRemains() < 2 and (Target:DebuffRemains(S.PhantomSingularityDebuff) > 2 or not S.PhantomSingularity:IsAvailable())) then
      local ShouldReturn = Darkglare_prep(); if ShouldReturn then return ShouldReturn; end
    end
    -- dark_soul,if=cooldown.summon_darkglare.remains>time_to_die
    if S.DarkSoulMisery:IsReady() and Target:Level() == Player:Level() + 2 and (Player:IsInRaidArea() or Player:IsInDungeonArea()) or Target:Level() == Player:Level() + 1 and not Player:IsInRaidArea() and not Player:IsInDungeonArea() and (S.SummonDarkglare:CooldownRemains() > Target:TimeToDie()) then
